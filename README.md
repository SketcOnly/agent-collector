...
topic: "agent-collector-data" # 转发目标 Topic
http:
...
```bash
# 直接启动
go run cmd/agent-collector/main.go
...
# 2. 启动服务（指定配置文件）
./bin/agent-collector --config configs/config.yaml

# 3. 后台运行（Linux/macOS）
nohup ./bin/agent-collector --config configs/config.yaml > ./logs/agent-collector.log 2>&1 &
```
