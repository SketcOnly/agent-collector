// Package logger logger包基于zap和file-rotatelogs实现高性能日志工具，支持以下核心特性：
// 1. 双输出目标：控制台彩色格式化输出 + 文件JSON格式持久化
// 2. 日志轮转：按天自动轮转日志文件，保留7天历史日志
// 3. 级别过滤：支持debug/info/warn/error/panic/fatal六级日志过滤
// 4. 增强可读性：控制台输出带颜色区分（时间蓝/级别多色/调用者精简路径）
// 5. 线程安全：无并发安全风险（已移除共享状态字段）
// 6. 调试友好：错误级别日志自动附加堆栈信息，调用者信息包含文件路径+行号
//
// 使用规范：
// 1. 程序启动时必须先调用Init()初始化，传入日志配置
// 2. 日志输出优先使用包暴露的Debug/Info/Warn等方法
// 3. 程序退出前建议调用Sync()确保日志缓冲区数据刷盘
package logger
...
// InitLogger 初始化日志系统，必须在使用任何日志方法前调用（建议程序启动时执行）
// 参数cfg：日志配置结构体，包含日志级别、存储路径等核心配置
// 返回值：初始化过程中产生的错误（如目录创建失败、日志轮转器初始化失败等）
func InitLogger(cfg *config.ZapLogConfig) (*zap.Logger, error) {
	var err error
	loggerInitOnce.Do(func() {
		...
		coloredLevelEncoder := func(level zapcore.Level, enc zapcore.PrimitiveArrayEncoder) {
			...
			case zapcore.DebugLevel:
				levelStr = "\033[36mDBG \033[0m" // 蓝色DBG
			case zapcore.InfoLevel:
				levelStr = "\033[32mINF \033[0m" // 绿色INF
			case zapcore.WarnLevel:
				levelStr = "\033[33mWAR \033[0m" // 黄色WAR
			case zapcore.ErrorLevel:
				levelStr = "\033[31mERR \033[0m" // 红色ERR
...
// GetGlobalLogger 获取基础的zap.Logger实例（包含默认配置）
